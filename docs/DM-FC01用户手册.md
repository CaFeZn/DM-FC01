...
- **Flash**：2 MB
- **RAM**：1 MB
- **外部 Flash**：16 MB W25Q128JVSQ（用于数据记录）
...
![正面接口定义](images/正面接口定义.png)

![背面接口定义](images/背面接口定义.png)
...
| 1    | GND      | GND           | -          | 地         |
| 2    | VBAT     | ⚠️**电池电压** | -          | 电池输入   |
| 3    | CAN1_H   | 5.0V          | PD1        | CAN 高电平 |
| 4    | CAN1_L   | 5.0V          | PD0        | CAN 低电平 |
...
| 引脚 | 信号名称 | 电压 | STM32 引脚 | 功能     |
| ---- | -------- | ---- | ---------- | -------- |
| 1    | UART4_RX | 3.3V | PB8        | 串口接收 |
| 2    | GND      | GND  | -          | 地       |
| 3    | UART4_TX | 3.3V | PB9        | 串口发送 |

![正面UART4焊盘定义](images/正面UART4焊盘定义.png)

#### TELEM4 接口（3-pin 测试点）

| 引脚 | 信号名称 | 电压 | STM32 引脚 | 功能     |
| ---- | -------- | ---- | ---------- | -------- |
| 1    | UART6_RX | 3.3V | PC7        | 串口接收 |
| 2    | GND      | GND  | -          | 地       |
| 3    | UART6_TX | 3.3V | PC6        | 串口发送 |

![正面UART6焊盘定义](images/正面UART6焊盘定义.png)
...
![Debug接口定义](images/Debug接口定义.png)

#### AuxGPIO 接口（6-pin SH1.0）
...
| /dev/ttyS2 | TELEM2   | USART3 | MAVLink       |
| /dev/ttyS3 | TELEM3   | UART4  | MAVLink       |
| /dev/ttyS4 | RC_INPUT | UART5  | RCIN          |
| /dev/ttyS5 | URT6     | USART6 | -             |
...
| SERIAL4 | DJI O3        | USART3 | MAVLink       |
| SERIAL5 | RC_INPUT      | UART5  | RCIN          |
| SERIAL6 | ESC Telemetry | UART7  | ESC Telemetry |
| SERIAL7 | TELEM3        | UART4  | MAVLink       |
| SERIAL8 | TELEM4        | UART6  | MAVLink       |
...
| PWM5 | PB11       | TIM2_CH4 | Group2 |
| PWM6 | PB10       | TIM2_CH3 | Group2 |
| PWM7 | PB0        | TIM3_CH3 | Group3 |
| PWM8 | PB1        | TIM3_CH4 | Group3 |
...
**PX4：**
- 加热器默认已启用
- 可通过参数调整目标温度（QGC中搜索 `heater` 调整参数）

**Ardupilot：**
- 加热器在固件中已配置
- 可通过参数 `BRD_HEAT_TARG` 设置目标温度

### 7. 如何校准电池监控？
1. 使用万用表测量实际电池电压
2. 读取飞控显示的电压值
3. 计算校准系数：`新系数 = 旧系数 × (实际电压 / 显示电压)`
...
### 9. 如何使用外部 Flash 记录日志？
**PX4：**
- 暂不支持，目前仅支持SD卡记录与数据存储。PX4尚未实现该驱动

**Ardupilot：**
- 外部 Flash 会自动检测并用于日志记录
- 可通过 `LOG_BACKEND_TYPE` 参数配置
- SD 卡和 Flash 只能二选一

### 10. 如何连接外部磁力计？
- 外部磁力计可连接到 I2C1（板载气压计总线）或 I2C4（GPS 接口）
- 推荐连接到 GPS 模块的 I2C 接口（I2C4）
- 连接后需要进行磁力计校准
